class Lead < ActiveRecord::Base
	...
	scope :keyword, -> (keyword) { where("leads.title ILIKE ? OR leads.description ILIKE ? OR leads.name ILIKE ? OR leads.email ILIKE ? OR leads.notes ILIKE ?", "%#{keyword}%", "%#{keyword}%", "%#{keyword}%", "%#{keyword}%", "%#{keyword}%") unless keyword.blank? }
