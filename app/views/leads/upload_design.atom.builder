atom_feed do |feed|
  ...
  @leads.most_recent.take(1).each do |lead|
    feed.entry lead do |entry|
      if @exclusives.this_week.exists? then entry.title Time.now.strftime("Latest Gigs %d %B %Y **exclusive lead inside**") else entry.title Time.now.strftime("Latest Gigs %d %B %Y") end
